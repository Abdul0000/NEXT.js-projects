...
const app = new Hono()
    ...
    .delete("/:memberId",sessionMiddleware,async(c)=>{
        ...
        if(allMembersInWorkspace.total === 1)
            {
                return c.json({error:"Cannot delete the only"}, 401)
            }
        if(!member){
            return c.json({error:"Unauthorized"}, 401)
        }
        if(member.$id !== memberToDelete.$id && member.rol !== MemberRole.ADMIN)
        {
            return c.json({error:"Unauthorized"}, 401)
        }
    ...
    async(c)=>{
        ...
        if(member.rol !== MemberRole.ADMIN)
        {
            return c.json({error:"Unauthorized"}, 401)
        }
