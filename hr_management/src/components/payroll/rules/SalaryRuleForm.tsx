...
const SalaryRuleForm = ({ rule_id }: EmployeeContractFormProps) => {
  ...
  const updateSalaryRule = trpc.payrollSalaryRule.updateSalaryrule.useMutation({
    onSuccess: () => {
      ...
      utils.payrollSalaryRule.getOneSalaryRule.invalidate({ rule_id });
  ...
  const [getSalaryRule] = trpc.payrollSalaryRule.getOneSalaryRule.useSuspenseQuery({
    rule_id,
    // code?: "", 
    // rule_name: "",
  });
  ...
  const onSubmit = async (data: z.infer<typeof insertSalaryRulesSchema>) => {
    if (getSalaryRule) {
      updateSalaryRule.mutate({ ...data, rule_id });
    } else {
